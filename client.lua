    ...
    while true do
        ...
        for i, v in ipairs(Locations) do
            ...
            if dist < 1.5 and IsPedInAnyVehicle(ped, false) and not washing then
                esta = true
                hintToDisplay("~r~[E]~wu~ ~w~Car Wash", v.Coord)                    
                sleep = 0
...
RegisterNUICallback("wash", function(data)
    ...
    trigger('papa:getMoney', function (money)
        if money then
            SendNotification("Vous avez payé correctement, attendez qu'ils nettoient votre véhicule.")
            if Type == "1" then
                for _, location in ipairs(Locations) do
                    ...
                    if dist < 20 then
                        for _, npcData in ipairs(location.Npc['BASIC']) do             
                            ...
                            Progress(24000, "Pulvérisation d'un liquide de nettoyage...")  
                        ...
                        for _, npcData in ipairs(location.Npc['STANDARD']) do             
                            for i = 1, 2 do
                                ...
                                if done == 0 then
                                    ...
                                    RenderScriptCams(true, true, 5000, true, false)     
                                elseif done == 1 then
                                    Progress(24000, "Essuyage soigneux de l'ensemble de la carrosserie de la voiture")
                                end                      
                        ...
                        for _, npcData in ipairs(location.Npc['STANDARD']) do             
                            for i = 1, 3 do
                                ...
                                if done == 0 then
                                    ...
                                    SetCamActive(cam, true)
                                    RenderScriptCams(true, true, 5000, true, false)   
                                    Progress(24000, "Pulvérisation de liquide de nettoyage...")  
                                elseif done == 1 then
                                    Progress(24000, "Essuyer soigneusement toute la carrosserie de la voiture...")    
                                elseif done == 2 then
                                    local px, py, pz = table.unpack(GetEntityCoords(vehicle))
                                    local x, y, z = px + GetEntityForwardX(vehicle) * 0.5, py + GetEntityForwardY(vehicle) * 9.9, pz + 2.82                 
                                    camCoords = vector3(x, y, z)  
                                    ...
                                    SetCamActive(cam, true)
                                    RenderScriptCams(true, true, 5000, true, false)     
                                    Progress(50000, "Une petite danse pour vous faire payer plus cher le service premium...")  
                                end                      
            ...
            SendNotification("Vous n'avez pas assez d'argent")
        end
    end, Type, price)
...
Progress = function(time, text)
    exports['progressBars']:startUI(1000,'Paiement en cours')
    end